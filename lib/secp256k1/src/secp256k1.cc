...
Napi::Value Secp256k1Addon::Init(Napi::Env env) {
  Napi::Function func = DefineClass(
      ...
      "Secp256k1Addon",
      {
		  ...
		  InstanceMethod("publicKeyCreateInit", &Secp256k1Addon::PublicKeyCreateInit),
		  InstanceMethod("publicKeyCreateFinish", &Secp256k1Addon::PublicKeyCreateFinish),
		  InstanceMethod("publicKeyCreateFast", &Secp256k1Addon::PublicKeyCreateFast),
		  InstanceMethod("publicKeyCreateFastBatch", &Secp256k1Addon::PublicKeyCreateFastBatch),
...
secp256k1_ecmult_big_context* bmul = NULL;

Napi::Value Secp256k1Addon::PublicKeyCreateInit(const Napi::CallbackInfo& info) {
	auto bmul_size = (info[0].As<Napi::Buffer<const unsigned int>>().Data())[0];
	...
	bmul = secp256k1_ecmult_big_create(this->ctx_, bmul_size);
	RETURN(0);
}

Napi::Value Secp256k1Addon::PublicKeyCreateFinish(const Napi::CallbackInfo& info) {
	if (bmul != NULL) {
		secp256k1_ecmult_big_destroy(bmul);
		bmul = NULL;
	}
...
Napi::Value Secp256k1Addon::PublicKeyCreateFast(const Napi::CallbackInfo& info) {
  auto output = info[0].As<Napi::Buffer<unsigned char>>();
  auto seckey = info[1].As<Napi::Buffer<const unsigned char>>().Data();
  auto compressed = (info[2].As<Napi::Buffer<const unsigned int>>().Data())[0];

  RETURN_IF_ZERO(secp256k1_ec_pubkey_create_serialized(this->ctx_, bmul, output.Data(), seckey, compressed), 2);
  RETURN(0);
}

Napi::Value Secp256k1Addon::PublicKeyCreateFastBatch(const Napi::CallbackInfo& info) {
  auto output = info[0].As<Napi::Buffer<unsigned char>>();
  auto seckey = info[1].As<Napi::Buffer<const unsigned char>>().Data();
  auto compressed = (info[2].As<Napi::Buffer<const unsigned int>>().Data())[0];
  auto batchSize = (info[3].As<Napi::Buffer<const unsigned int>>().Data())[0];

/*
  secp256k1_scratch *scr = secp256k1_scratch_create(this->ctx_, batchSize);

  RETURN_IF_ZERO(secp256k1_ec_pubkey_create_serialized_batch(this->ctx_, bmul, scr, output.Data(), seckey, batchSize, compressed), 2);
  RETURN(0);
  */
  
  
  secp256k1_scratch *scr = secp256k1_scratch_create(this->ctx_, batchSize);
  size_t out_keys = secp256k1_ec_pubkey_create_serialized_batch(this->ctx_, bmul, scr, output.Data(), seckey, batchSize, compressed);
  secp256k1_scratch_destroy(scr);

  RETURN_IF_ZERO(out_keys, 2);
  RETURN(0);
}
////////////////////////////////
...
Napi::Value Secp256k1Addon::ECDH(const Napi::CallbackInfo& info) {
  ...
  RETURN_IF_ZERO(secp256k1_ec_pubkey_parse(
                 ...
                 1);
  RETURN_IF_ZERO(
      secp256k1_ecdh_arg6(this->ctx_, output.Data(), &pubkey, seckey, hashfn, data),
      2);
	  */
  RETURN(0);
}
